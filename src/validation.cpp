...
CAmount GetBlockSubsidy(const CBlockIndex* const pindex, const Consensus::Params& consensusParams)
{
    ...
    bool fV20Active{llmq::utils::IsV20Active(pindex->pprev)};
    bool fMNRewardReallocated{llmq::utils::IsMNRewardReallocationActive(pindex->pprev)};
    return GetBlockSubsidyInner(pindex->pprev->nBits, pindex->pprev->nHeight, consensusParams, fV20Active, fMNRewardReallocated);
}
