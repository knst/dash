...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_chain_helper.CheckSpecialTx(tx, m_active_chainstate.m_chain.Tip(), m_active_chainstate.CoinsTip(), true, state))
        return false;
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!m_chain_helper->UndoSpecialTxsInBlock(block, pindex, mnlist_updates_opt)) {
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    std::optional<MNListUpdates> mnlist_updates_opt{std::nullopt};
    if (!m_chain_helper->ProcessSpecialTxsInBlock(block, pindex, view, fJustCheck, fScriptChecks, state, mnlist_updates_opt)) {
    ...
    if (!m_chain_helper->CheckCreditPoolDiffForBlock(block, pindex, blockSubsidy, state)) {
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    BlockValidationState state;
    std::optional<MNListUpdates> mnlist_updates_opt{std::nullopt};
    if (!m_chain_helper->ProcessSpecialTxsInBlock(block, pindex, inputs, false /*fJustCheck*/, false /*fScriptChecks*/, state, mnlist_updates_opt)) {
