...
[[nodiscard]] bool MNSubsidyAgent::GetBlockTxOuts(const CBlockIndex* pindexPrev, const CAmount blockSubsidy, const CAmount feeReward,
                                                  std::vector<CTxOut>& voutMasternodePaymentsRet)
{
    ...
    if (DeploymentActiveAfter(pindexPrev, m_consensus_params, Consensus::DEPLOYMENT_MN_RR)) {
        ...
        LogPrint(BCLog::MNPAYMENTS, "MNSubsidyAgent::%s(): MN reward %lld reallocated to credit pool\n", __func__, platformReward);
...
*/
[[nodiscard]] bool MNSubsidyAgent::GetMasternodeTxOuts(const CBlockIndex* pindexPrev, const CAmount blockSubsidy, const CAmount feeReward,
                                                       std::vector<CTxOut>& voutMasternodePaymentsRet)
{
    ...
    if(!GetBlockTxOuts(pindexPrev, blockSubsidy, feeReward, voutMasternodePaymentsRet)) {
        LogPrintf("MNSubsidyAgent::%s(): No payee (deterministic masternode list empty)\n", __func__);
        return false;
    }
    ...
    for (const auto& txout : voutMasternodePaymentsRet) {
        ...
        LogPrintf("MNSubsidyAgent::%s(): Masternode payment %lld to %s\n", __func__, txout.nValue, EncodeDestination(dest));
    }

    return true;
}

[[nodiscard]] bool MNSubsidyAgent::IsTransactionValid(const CTransaction& txNew, const CBlockIndex* pindexPrev, const CAmount blockSubsidy,
                                                      const CAmount feeReward)
{
    ...
    if (!GetBlockTxOuts(pindexPrev, blockSubsidy, feeReward, voutMasternodePayments)) {
        LogPrintf("MNSubsidyAgent::%s(): ERROR! Failed to get payees for block at height %s\n", __func__, nBlockHeight);
        return true;
    }
    ...
    for (const auto& txout : voutMasternodePayments) {
        ...
        if (!found) {
            ...
            if (!ExtractDestination(txout.scriptPubKey, dest))
                assert(false);
            LogPrintf("MNSubsidyAgent::%s(): ERROR! Failed to find expected payee %s in block at height %s\n", __func__, EncodeDestination(dest), nBlockHeight);
            return false;
        }
    }
    return true;
...
bool MNSubsidyAgent::IsOldBudgetBlockValueValid(const CBlock& block, const int nBlockHeight, const CAmount blockReward, std::string& strErrorRet)
{
    ...
    if (nOffset < m_consensus_params.nBudgetPaymentsWindowBlocks) {
        ...
        if(m_mn_sync.IsSynced()) {
            ...
            LogPrint(BCLog::GOBJECT, "MNSubsidyAgent::%s(): WARNING! Client synced but old budget system is disabled, checking block value against block reward\n", __func__);
        ...
        LogPrint(BCLog::GOBJECT, "MNSubsidyAgent::%s(): WARNING! Skipping old budget block value checks, accepting block\n", __func__);
        return true;
    }
    if(!isBlockRewardValueMet) {
...
*/
bool MNSubsidyAgent::IsBlockValueValid(const CBlock& block, const int nBlockHeight, const CAmount blockReward, std::string& strErrorRet)
{
    bool isBlockRewardValueMet = (block.vtx[0]->GetValueOut() <= blockReward);
    ...
    if(!m_mn_sync.IsSynced() || fDisableGovernance) {
        LogPrint(BCLog::MNPAYMENTS, "MNSubsidyAgent::%s(): WARNING! Not enough data, checked superblock max bounds only\n", __func__);
    ...
    if (!AreSuperblocksEnabled(m_sporkman)) {
        ...
        LogPrint(BCLog::GOBJECT, "MNSubsidyAgent::%s(): Superblocks are disabled, no superblocks allowed\n", __func__);
        if(!isBlockRewardValueMet) {
    ...
    if (!CSuperblockManager::IsValid(m_govman, *block.vtx[0], nBlockHeight, blockReward)) {
        ...
        LogPrintf("MNSubsidyAgent::%s(): ERROR! Invalid superblock detected at height %d: %s", __func__, nBlockHeight, block.vtx[0]->ToString()); /* Continued */
...
bool MNSubsidyAgent::IsBlockPayeeValid(const CTransaction& txNew, const CBlockIndex* pindexPrev, const CAmount blockSubsidy, const CAmount feeReward)
{
    const int nBlockHeight = pindexPrev  == nullptr ? 0 : pindexPrev->nHeight + 1;

    // Check for correct masternode payment
    if (IsTransactionValid(txNew, pindexPrev, blockSubsidy, feeReward)) {
        LogPrint(BCLog::MNPAYMENTS, "MNSubsidyAgent::%s(): Valid masternode payment at height %d: %s", __func__, nBlockHeight, txNew.ToString()); /* Continued */
    } else {
        LogPrintf("MNSubsidyAgent::%s(): ERROR! Invalid masternode payment detected at height %d: %s", __func__, nBlockHeight, txNew.ToString()); /* Continued */
        return false;
    }

    if (!m_mn_sync.IsSynced() || fDisableGovernance) {
        // governance data is either incomplete or non-existent
        LogPrint(BCLog::MNPAYMENTS, "MNSubsidyAgent::%s(): WARNING! Not enough data, skipping superblock payee checks\n", __func__);
    ...
    if (nBlockHeight < m_consensus_params.nSuperblockStartBlock) {
        ...
        LogPrint(BCLog::GOBJECT, "MNSubsidyAgent::%s(): WARNING! Client synced but old budget system is disabled, accepting any payee\n", __func__);
        return true; // not an error
    }
    ...
    if (AreSuperblocksEnabled(m_sporkman)) {
        if (CSuperblockManager::IsSuperblockTriggered(m_govman, nBlockHeight)) {
            if (CSuperblockManager::IsValid(m_govman, txNew, nBlockHeight, blockSubsidy + feeReward)) {
                LogPrint(BCLog::GOBJECT, "MNSubsidyAgent::%s(): Valid superblock at height %d: %s", __func__, nBlockHeight, txNew.ToString()); /* Continued */
                // continue validation, should also pay MN
            } else {
                LogPrintf("MNSubsidyAgent::%s(): ERROR! Invalid superblock detected at height %d: %s", __func__, nBlockHeight, txNew.ToString()); /* Continued */
                // should NOT allow such superblocks, when superblocks are enabled
                return false;
            }
        } else {
            LogPrint(BCLog::GOBJECT, "MNSubsidyAgent::%s(): No triggered superblock detected at height %d\n", __func__, nBlockHeight);
        }
    } else {
        // should NOT allow superblocks at all, when superblocks are disabled
        LogPrint(BCLog::GOBJECT, "MNSubsidyAgent::%s(): Superblocks are disabled, no superblocks allowed\n", __func__);
...
void MNSubsidyAgent::FillBlockPayments(CMutableTransaction& txNew, const CBlockIndex* pindexPrev, const CAmount blockSubsidy, const CAmount feeReward,
                                       std::vector<CTxOut>& voutMasternodePaymentsRet, std::vector<CTxOut>& voutSuperblockPaymentsRet)
{
    ...
    if(AreSuperblocksEnabled(m_sporkman) && CSuperblockManager::IsSuperblockTriggered(m_govman, nBlockHeight)) {
        LogPrint(BCLog::GOBJECT, "MNSubsidyAgent::%s(): Triggered superblock creation at height %d\n", __func__, nBlockHeight);
    ...
    if (!GetMasternodeTxOuts(pindexPrev, blockSubsidy, feeReward, voutMasternodePaymentsRet)) {
        LogPrint(BCLog::MNPAYMENTS, "MNSubsidyAgent::%s(): No masternode to pay (MN list probably empty)\n", __func__);
    }
    ...
    LogPrint(BCLog::MNPAYMENTS, "MNSubsidyAgent::%s(): nBlockHeight %d blockReward %lld voutMasternodePaymentsRet \"%s\" txNew %s", __func__, /* Continued */
