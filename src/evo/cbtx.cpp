...
bool CalcCbTxMerkleRootMNList(uint256& merkleRootRet, const CDeterministicMNList& mn_list, BlockValidationState& state)
{
    try {
        ...
        static std::shared_ptr<const CSimplifiedMNList> cached_sml{
            std::make_shared<const CSimplifiedMNList>()} GUARDED_BY(cached_mutex);
...
auto CachedGetQcHashesQcIndexedHashes(const CBlockIndex* pindexPrev, const llmq::CQuorumBlockProcessor& quorum_block_processor) ->
    ...
    static std::map<Consensus::LLMQType, unordered_lru_cache<uint256, std::pair<uint256, int>, StaticSaltedHasher>> qc_hashes_cached GUARDED_BY(minableCommitmentsCs);
