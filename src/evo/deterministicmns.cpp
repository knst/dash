...
void CDeterministicMNList::UpdateMN(const CDeterministicMN& oldDmn, const std::shared_ptr<const CDeterministicMNState>& pdmnState)
{
    ...
    if (!UpdateUniquePropertyOrTransform(*dmn, oldState->pubKeyOperator, pdmnState->pubKeyOperator)) {
        mnUniquePropertyMap = mnUniquePropertyMapSaved;
...
void CDeterministicMNList::RemoveMN(const uint256& proTxHash)
{
    ...
    LogPrintf("%s(): pubKeyOperator is %s, pk: %s\n", __func__, dmn->pdmnState->pubKeyOperator.Get().IsValid() ? "valid" : "invalid", dmn->pdmnState->pubKeyOperator.Get().ToString());
