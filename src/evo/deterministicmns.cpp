...
void CDeterministicMNManager::HandleQuorumCommitment(const llmq::CFinalCommitment& qc, const CBlockIndex* pQuorumBaseBlockIndex, CDeterministicMNList& mnList, bool debugLogs)
{
    ...
    auto members = GetAllQuorumMembers(qc.llmqType, pQuorumBaseBlockIndex, /*reset_cache=*/ false); /// step-2
...
/// depends on global quorumSnapshot
static void BuildQuorumSnapshot(const Consensus::LLMQParams& llmqParams, const CDeterministicMNList& allMns,
                                                 ...
                                                 llmq::CQuorumSnapshot& quorumSnapshot, int nHeight, std::vector<int>& skipList, const CBlockIndex* pCycleQuorumBaseBlockIndex)
{
    ...
    if (skipList.empty()) {
        quorumSnapshot.mnSkipListMode = llmq::SnapshotSkipMode::MODE_NO_SKIPPING;
        quorumSnapshot.mnSkipList.clear();
    } else {
        quorumSnapshot.mnSkipListMode = llmq::SnapshotSkipMode::MODE_SKIPPING_ENTRIES;
        quorumSnapshot.mnSkipList = std::move(skipList);
    }
}

std::vector<std::vector<CDeterministicMNCPtr>> CDeterministicMNManager::BuildNewQuorumQuarterMembers(const Consensus::LLMQParams& llmqParams,
                                                                                                     ...
                                                                                                     const PreviousQuorumQuarters& previousQuarters)
{
    ...
    bool skipRemovedMNs = llmq::utils::IsV19Active(pCycleQuorumBaseBlockIndex) || (Params().NetworkIDString() == CBaseChainParams::TESTNET);
    ...
    llmq::quorumSnapshotManager->StoreSnapshotForBlock(llmqParams.type, pCycleQuorumBaseBlockIndex, quorumSnapshot);

    return quarterQuorumMembers;
}

std::pair<CDeterministicMNList, CDeterministicMNList> CDeterministicMNManager::GetMNUsageBySnapshot(const Consensus::LLMQParams& llmqParams,
                                                                                       const CBlockIndex* pCycleQuorumBaseBlockIndex,
...
std::vector<std::vector<CDeterministicMNCPtr>> CDeterministicMNManager::GetQuorumQuarterMembersBySnapshot(const Consensus::LLMQParams& llmqParams,
                                                                                             const CBlockIndex* pCycleQuorumBaseBlockIndex,
                                                                                             const llmq::CQuorumSnapshot& snapshot,
                                                                                             int nHeight)
{
    ...
    switch (snapshot.mnSkipListMode) {
        case llmq::SnapshotSkipMode::MODE_NO_SKIPPING:
        {
        ...
        case llmq::SnapshotSkipMode::MODE_SKIPPING_ENTRIES: // List holds entries to be skipped
        {
        ...
        case llmq::SnapshotSkipMode::MODE_NO_SKIPPING_ENTRIES: // List holds entries to be kept
        case llmq::SnapshotSkipMode::MODE_ALL_SKIPPED: // Every node was skipped. Returning empty quarterQuorumMembers
        default:
            return quarterQuorumMembers;
    }
}
...
PreviousQuorumQuarters CDeterministicMNManager::GetPreviousQuorumQuarterMembers(const Consensus::LLMQParams& llmqParams,
                                                                                ...
                                                                                int nHeight)
{
    ...
    std::optional<llmq::CQuorumSnapshot> quSnapshotHMinusC = llmq::quorumSnapshotManager->GetSnapshotForBlock(llmqParams.type, pBlockHMinusCIndex);
    if (quSnapshotHMinusC.has_value()) {
        ...
        std::optional<llmq::CQuorumSnapshot> quSnapshotHMinus2C = llmq::quorumSnapshotManager->GetSnapshotForBlock(llmqParams.type, pBlockHMinus2CIndex);
        if (quSnapshotHMinus2C.has_value()) {
            ...
            std::optional<llmq::CQuorumSnapshot> quSnapshotHMinus3C = llmq::quorumSnapshotManager->GetSnapshotForBlock(llmqParams.type, pBlockHMinus3CIndex);
...
std::vector<std::vector<CDeterministicMNCPtr>> CDeterministicMNManager::ComputeQuorumMembersByQuarterRotation(const Consensus::LLMQParams& llmqParams, const CBlockIndex* pCycleQuorumBaseBlockIndex)
{
...
std::vector<CDeterministicMNCPtr> CDeterministicMNManager::ComputeQuorumMembers(Consensus::LLMQType llmqType, const CBlockIndex* pQuorumBaseBlockIndex)
{
    bool EvoOnly = (Params().GetConsensus().llmqTypePlatform == llmqType) && llmq::utils::IsV19Active(pQuorumBaseBlockIndex);
    ...
    const CBlockIndex* pWorkBlockIndex = llmq::utils::IsV20Active(pQuorumBaseBlockIndex) ?
...
std::vector<CDeterministicMNCPtr> CDeterministicMNManager::GetAllQuorumMembers(Consensus::LLMQType llmqType, const CBlockIndex* pQuorumBaseBlockIndex, bool reset_cache)
{
    ...
    if (!llmq::utils::IsQuorumTypeEnabled(llmqType, *llmq::quorumManager, pQuorumBaseBlockIndex->pprev)) {
        return {};
    }
    std::vector<CDeterministicMNCPtr> quorumMembers;
    {
        ...
        if (mapQuorumMembers.empty()) {
            llmq::utils::InitQuorumsCache(mapQuorumMembers);
        }
        if (reset_cache) {
    ...
    if (llmq::utils::IsQuorumRotationEnabled(llmq_params, pQuorumBaseBlockIndex)) {
        if (LOCK(cs_indexed_members); mapIndexedQuorumMembers.empty()) {
            llmq::utils::InitQuorumsCache(mapIndexedQuorumMembers);
        }
        /*
        ...
        auto q = ComputeQuorumMembersByQuarterRotation(llmq_params, pCycleQuorumBaseBlockIndex); // step-3
        LOCK(cs_indexed_members);
