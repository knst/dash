...
private:
    ...
    // This cache is used to improve performance and coulde be reused
    // for multple CDeterministicMNList until list is actually changed.
    // AddMN, RemoveMN and (in some cases) UpdateMN reset this cache; including indirect calls
    // such as ApplyDiff
...
public:
     ...
     */
    std::shared_ptr<const CSimplifiedMNList> to_sml() const;
...
private:
    ...
    std::unordered_map<uint256, std::shared_ptr<const CDeterministicMNList>, StaticSaltedHasher> mnListsCache GUARDED_BY(cs);
...
private:
    ...
    std::shared_ptr<const CDeterministicMNList> GetListForBlockInternal(gsl::not_null<const CBlockIndex*> pindex) EXCLUSIVE_LOCKS_REQUIRED(cs);
};
