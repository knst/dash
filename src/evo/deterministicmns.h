...
private:
    ...
    [[nodiscard]] bool DeleteUniquePropertyOrTransform(const CDeterministicMN& dmn, const CBLSLazyPublicKey& oldValue)
    {
        ...
        if (p == nullptr) {
            ...
            LogPrintf("%s(): attempting switch and revalidate, legacy: %s, basic: %s\n", __func__, _oldHash.ToString(), oldHash.ToString());
            if (p == nullptr) {
                LogPrintf("%s(): revalidate failed, still not found in mnUniquePropertyMap, reverting back\n", __func__);
                bls::switchSignatureSchemeState();
                return false;
            }
        }
        if (p->first != dmn.proTxHash) {
            if (switch_scheme) { 
                LogPrintf("%s(): revalidate failed, %s != %s, reverting back\n", __func__, p->first.ToString(), dmn.proTxHash.ToString());
                bls::switchSignatureSchemeState();
            }
            return false;
        }
        ...
        if (switch_scheme) { 
            LogPrintf("%s(): revalidate successful under %s scheme, switching back\n", __func__, bls::getSignatureSchemeState());
            bls::switchSignatureSchemeState();
        }
        return true;
    }
