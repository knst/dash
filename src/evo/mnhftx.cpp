...
std::optional<CMNHFManager::Signals> CMNHFManager::GetFromCache(const CBlockIndex* const pindex)
{
    ...
    {
        LOCK(cs_cache);
        if (!DeploymentActiveAfter(pindex->pprev, Params().GetConsensus(), Consensus::DEPLOYMENT_V20)) {
...
void CMNHFManager::AddToCache(const Signals& signals, const CBlockIndex* const pindex)
{
    ...
    {
        LOCK(cs_cache);
        if (!DeploymentActiveAfter(pindex->pprev, Params().GetConsensus(), Consensus::DEPLOYMENT_V20)) return;
    }
