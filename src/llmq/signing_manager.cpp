...
#include <llmq/signing_manager.h>
...
SingingManagerImpl::SingingManagerImpl(CConnman& _connman, const CActiveMasternodeManager* const mn_activeman, const CChainState& chainstate,
...
bool SingingManagerImpl::AlreadyHave(const CInv& inv) const
{
...
bool SingingManagerImpl::GetRecoveredSigForGetData(const uint256& hash, CRecoveredSig& ret) const
{
...
PeerMsgRet SingingManagerImpl::ProcessMessage(const CNode& pfrom, const std::string& msg_type, CDataStream& vRecv)
{
...
static bool PreVerifyRecoveredSig(const CQuorumManager& quorum_manager, const CRecoveredSig& recoveredSig, bool& retBan)
{
    ...
    if (!quorum) {
        LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- quorum %s not found\n", __func__,
...
PeerMsgRet SingingManagerImpl::ProcessMessageRecoveredSig(const CNode& pfrom, const std::shared_ptr<const CRecoveredSig>& recoveredSig)
{
    ...
    LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- signHash=%s, id=%s, msgHash=%s, node=%d\n", __func__,
    ...
    if (pendingReconstructedRecoveredSigs.count(recoveredSig->GetHash())) {
        ...
        LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- already pending reconstructed sig, signHash=%s, id=%s, msgHash=%s, node=%d\n", __func__,
                 recoveredSig->buildSignHash().ToString(), recoveredSig->getId().ToString(), recoveredSig->getMsgHash().ToString(), pfrom.GetId());
        return {};
    }
...
void SingingManagerImpl::CollectPendingRecoveredSigsToVerify(
    ...
    for (auto& p : retSigShares) {
        ...
        for (auto it = v.begin(); it != v.end();) {
            ...
            if (!retQuorums.count(quorumKey)) {
                ...
                if (!quorum) {
                    LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- quorum %s not found, node=%d\n", __func__,
                              recSig->getQuorumHash().ToString(), nodeId);
                    it = v.erase(it);
                    continue;
                }
                if (!IsQuorumActive(llmqType, qman, quorum->qc->quorumHash)) {
                    LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- quorum %s not active anymore, node=%d\n", __func__,
                              recSig->getQuorumHash().ToString(), nodeId);
                    it = v.erase(it);
                    continue;
                }
...
void SingingManagerImpl::ProcessPendingReconstructedRecoveredSigs()
{
...
bool SingingManagerImpl::ProcessPendingRecoveredSigs()
{
    ...
    LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- verified recovered sig(s). count=%d, vt=%d, nodes=%d\n", __func__, verifyCount, verifyTimer.count(), recSigsByNode.size());
    ...
    for (const auto& p : recSigsByNode) {
        ...
        if (batchVerifier.badSources.count(nodeId)) {
            LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- invalid recSig from other node, banning peer=%d\n", __func__, nodeId);
...
void SingingManagerImpl::ProcessRecoveredSig(const std::shared_ptr<const CRecoveredSig>& recoveredSig)
{
    auto llmqType = recoveredSig->getLlmqType();
    ...
    LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- valid recSig. signHash=%s, id=%s, msgHash=%s\n", __func__,
    ...
    if (db.HasRecoveredSigForId(llmqType, recoveredSig->getId())) {
        ...
        if (db.GetRecoveredSigById(llmqType, recoveredSig->getId(), otherRecoveredSig)) {
            ...
            if (signHash != otherSignHash) {
                ...
                LogPrintf("SingingManagerImpl::%s -- conflicting recoveredSig for signHash=%s, id=%s, msgHash=%s, otherSignHash=%s\n", __func__,
...
void SingingManagerImpl::PushReconstructedRecoveredSig(const std::shared_ptr<const llmq::CRecoveredSig>& recoveredSig)
{
    LOCK(cs_pending);
...
void SingingManagerImpl::TruncateRecoveredSig(Consensus::LLMQType llmqType, const uint256& id)
{
    db.TruncateRecoveredSig(llmqType, id);
}

void SingingManagerImpl::Cleanup()
{
...
void SingingManagerImpl::RegisterRecoveredSigsListener(CRecoveredSigsListener* l)
{
    LOCK(cs_listeners);
    recoveredSigsListeners.emplace_back(l);
}

void SingingManagerImpl::UnregisterRecoveredSigsListener(CRecoveredSigsListener* l)
{
    LOCK(cs_listeners);
...
bool SingingManagerImpl::AsyncSignIfMember(Consensus::LLMQType llmqType, CSigSharesManager& shareman, const uint256& id,
                                        ...
                                        bool allowDiffMsgHashSigning)
{
    ...
    if (!quorum) {
        LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- failed to select quorum. id=%s, msgHash=%s\n", __func__, id.ToString(), msgHash.ToString());
        return false;
    }
        ...
        if (hasVoted) {
            ...
            if (msgHash != prevMsgHash) {
                if (allowDiffMsgHashSigning) {
                    LogPrintf("SingingManagerImpl::%s -- already voted for id=%s and msgHash=%s. Signing for different msgHash=%s\n",
                              __func__, id.ToString(), prevMsgHash.ToString(), msgHash.ToString());
                    hasVoted = false;
                } else {
                    LogPrintf("SingingManagerImpl::%s -- already voted for id=%s and msgHash=%s. Not voting on conflicting msgHash=%s\n",
                              __func__, id.ToString(), prevMsgHash.ToString(), msgHash.ToString());
                    return false;
                }
            } else if (allowReSign) {
                LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- already voted for id=%s and msgHash=%s. Resigning!\n", __func__,
                         id.ToString(), prevMsgHash.ToString());
            } else {
                LogPrint(BCLog::LLMQ, "SingingManagerImpl::%s -- already voted for id=%s and msgHash=%s. Not voting again.\n", __func__,
                          id.ToString(), prevMsgHash.ToString());
                return false;
            }
        }
...
bool SingingManagerImpl::HasRecoveredSig(Consensus::LLMQType llmqType, const uint256& id, const uint256& msgHash) const
{
    return db.HasRecoveredSig(llmqType, id, msgHash);
}

bool SingingManagerImpl::HasRecoveredSigForId(Consensus::LLMQType llmqType, const uint256& id) const
{
    return db.HasRecoveredSigForId(llmqType, id);
}

bool SingingManagerImpl::HasRecoveredSigForSession(const uint256& signHash) const
{
    return db.HasRecoveredSigForSession(signHash);
}

bool SingingManagerImpl::GetRecoveredSigForId(Consensus::LLMQType llmqType, const uint256& id, llmq::CRecoveredSig& retRecSig) const
{
...
bool SingingManagerImpl::IsConflicting(Consensus::LLMQType llmqType, const uint256& id, const uint256& msgHash) const
{
    if (!db.HasRecoveredSigForId(llmqType, id)) {
...
bool SingingManagerImpl::GetVoteForId(Consensus::LLMQType llmqType, const uint256& id, uint256& msgHashRet) const
{
    return db.GetVoteForId(llmqType, id, msgHashRet);
}

void SingingManagerImpl::StartWorkerThread()
{
...
void SingingManagerImpl::StopWorkerThread()
{
...
void SingingManagerImpl::InterruptWorkerThread()
{
    workInterrupt();
}
...
void SingingManagerImpl::WorkThreadMain()
{
    while (!workInterrupt) {
