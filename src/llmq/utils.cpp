...
static std::vector<std::pair<arith_uint256, CDeterministicMNCPtr>> CalculateScoresForQuorum(const std::vector<CDeterministicMNCPtr>& dmns, const uint256& modifier, const bool onlyEvoNodes)
{
...
static std::vector<std::pair<arith_uint256, CDeterministicMNCPtr>> CalculateScoresForQuorum(const CDeterministicMNList& mn_list, const uint256& modifier, const bool onlyEvoNodes)
{
    std::vector<std::pair<arith_uint256, CDeterministicMNCPtr>> scores;
...
static std::vector<CDeterministicMNCPtr> CalculateQuorum(const List& mn_list, size_t maxSize, const uint256& modifier, const bool onlyEvoNodes)
{
                                                      ...
                                                      bool reset_cache)
{
    ...
    if (IsQuorumRotationEnabled(llmq_params, pQuorumBaseBlockIndex)) {
        ...
        for (const size_t i : irange::range(q.size())) {
            mapIndexedQuorumMembers[llmqType].emplace(std::make_pair(pCycleQuorumBaseBlockIndex->GetBlockHash(), i), std::move(q[i]));
        }
    } else {
    ...
    const CBlockIndex* pCycleQuorumBaseBlockIndex, const llmq::CQuorumSnapshot& snapshot, int nHeight)
{
    ...
    std::vector<CDeterministicMNCPtr> sortedCombinedMns;
    {
        ...
        auto sortedMnsNotUsedAtH = CalculateQuorum(MnsNotUsedAtH, MnsNotUsedAtH.GetAllMNsCount(), modifier, false);
        sortedCombinedMns = std::move(sortedMnsNotUsedAtH);
        // Now add the already used MNs to the end of the list
        auto sortedMnsUsedAtH = CalculateQuorum(MnsUsedAtH, MnsUsedAtH.GetAllMNsCount(), modifier, false);
