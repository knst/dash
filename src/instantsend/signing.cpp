...
bool CSigningManager::IsInstantSendMempoolSigningEnabled() const
{
    return !fReindex && !fImporting && m_sporkman.GetSporkValue(SPORK_2_INSTANTSEND_ENABLED) == 0;
}

void CSigningManager::HandleNewInputLockRecoveredSig(const llmq::CRecoveredSig& recoveredSig, const uint256& txid)
{
    if (g_txindex) {
    ...
    if (LogAcceptDebug(BCLog::INSTANTSEND)) {
        for (const auto& in : tx->vin) {
            ...
            if (id == recoveredSig.getId()) {
                LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: got recovered sig for input %s\n", __func__,
...
void CSigningManager::ProcessPendingRetryLockTxs(const std::vector<CTransactionRef>& retryTxs)
{
    ...
    for (const auto& tx : retryTxs) {
        ...
        if (LogAcceptDebug(BCLog::INSTANTSEND)) {
            ...
            LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: retrying to lock\n", __func__,
                     tx->GetHash().ToString());
        }
...
bool CSigningManager::CheckCanLock(const CTransaction& tx, bool printDebug, const Consensus::Params& params) const
{
    if (tx.vin.empty()) {
...
bool CSigningManager::CheckCanLock(const COutPoint& outpoint, bool printDebug, const uint256& txHash, const Consensus::Params& params) const
{
    ...
    if (mempoolTx) {
        if (printDebug) {
            LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: parent mempool TX %s is not locked\n", __func__,
    ...
    if (!tx) {
        if (printDebug) {
            LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: failed to find parent TX %s\n", __func__,
                     txHash.ToString(), outpoint.hash.ToString());
        }
        return false;
    }
    ...
    if (nTxAge < nInstantSendConfirmationsRequired && !m_clhandler.HasChainLock(pindexMined->nHeight, pindexMined->GetBlockHash())) {
        if (printDebug) {
            LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: outpoint %s too new and not ChainLocked. nTxAge=%d, nInstantSendConfirmationsRequired=%d\n", __func__,
...
void CSigningManager::HandleNewInstantSendLockRecoveredSig(const llmq::CRecoveredSig& recoveredSig)
{
    ...
    if (islock->txid != recoveredSig.getMsgHash()) {
        LogPrintf("instantsend::CSigningManager::%s -- txid=%s: islock conflicts with %s, dropping own version\n", __func__,
...
void CSigningManager::ProcessTx(const CTransaction& tx, bool fRetroactive, const Consensus::Params& params)
{
    ...
    if (!CheckCanLock(tx, true, params)) {
        LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: CheckCanLock returned false\n", __func__,
                  tx.GetHash().ToString());
        return;
    }
    ...
    if (conflictingLock != nullptr) {
        ...
        LogPrintf("instantsend::CSigningManager::%s -- txid=%s: conflicts with islock %s, txid=%s\n", __func__,
...
bool CSigningManager::TrySignInputLocks(const CTransaction& tx, bool fRetroactive, Consensus::LLMQType llmqType, const Consensus::Params& params)
{
    ...
    for (const auto& in : tx.vin) {
        ...
        if (m_sigman.GetVoteForId(params.llmqTypeDIP0024InstantSend, id, otherTxHash)) {
            if (otherTxHash != tx.GetHash()) {
                LogPrintf("instantsend::CSigningManager::%s -- txid=%s: input %s is conflicting with previous vote for tx %s\n", __func__,
        ...
        if (m_sigman.IsConflicting(params.llmqTypeDIP0024InstantSend, id, tx.GetHash())) {
            LogPrintf("instantsend::CSigningManager::%s -- txid=%s: m_sigman.IsConflicting returned true. id=%s\n", __func__,
                      tx.GetHash().ToString(), id.ToString());
            return false;
        }
    }
    if (!fRetroactive && alreadyVotedCount == ids.size()) {
        LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: already voted on all inputs, bailing out\n", __func__,
                 tx.GetHash().ToString());
        return true;
    }

    LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: trying to vote on %d inputs\n", __func__,
    ...
    for (const auto i : irange::range(tx.vin.size())) {
        ...
        LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: trying to vote on input %s with id %s. fRetroactive=%d\n", __func__,
        ...
        if (m_sigman.AsyncSignIfMember(llmqType, m_shareman, id, tx.GetHash(), {}, fRetroactive)) {
            LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: voted on input %s with id %s\n", __func__,
...
void CSigningManager::TrySignInstantSendLock(const CTransaction& tx)
{
    ...
    LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- txid=%s: got all recovered sigs, creating CInstantSendLock\n", __func__,
            tx.GetHash().ToString());
    ...
    if (!quorum) {
        LogPrint(BCLog::INSTANTSEND, "instantsend::CSigningManager::%s -- failed to select quorum. islock id=%s, txid=%s\n",
