                                              ...
                                              const MnType mnType)
{
    ...
    bool isV19active = Params().GetConsensus().IsV19Active(WITH_LOCK(cs_main, return chainman.ActiveChain().Tip();));
...
static UniValue protx_update_service_common_wrapper(const JSONRPCRequest& request, const ChainstateManager& chainman, const MnType mnType)
{
    ...
    const bool isV19active = Params().GetConsensus().IsV19Active(WITH_LOCK(cs_main, return chainman.ActiveChain().Tip();));
...
static UniValue protx_update_registrar_wrapper(const JSONRPCRequest& request, const ChainstateManager& chainman, const bool specific_legacy_bls_scheme)
{
    ...
    const bool use_legacy = Params().GetConsensus().IsV19Active(chainman.ActiveChain().Tip()) ? specific_legacy_bls_scheme : true;
...
static UniValue bls_generate(const JSONRPCRequest& request, const ChainstateManager& chainman)
{
    ...
    CBLSSecretKey sk;
    sk.MakeNewKey();
    bool bls_legacy_scheme = !Params().GetConsensus().IsV19Active(chainman.ActiveChain().Tip());
    if (!request.params[0].isNull()) {
