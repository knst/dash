...
static UniValue generateBlocks(ChainstateManager& chainman, CEvoDB& evodb, LLMQContext& llmq_ctx, MNSubsidyAgent& mn_subsidy,
                               const CTxMemPool& mempool, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries)
{
    ...
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(chainman.ActiveChainstate(), evodb, llmq_ctx, mn_subsidy, mempool, Params()).CreateNewBlock(coinbase_script));
...
static UniValue generatetodescriptor(const JSONRPCRequest& request)
{
    ...
    return generateBlocks(chainman, *node.evodb, llmq_ctx, *node.mn_subsidy, mempool, coinbase_script, num_blocks, max_tries);
}
...
static UniValue generateblock(const JSONRPCRequest& request)
{
    ...
    CBlock block;
    {
        ...
        std::unique_ptr<CBlockTemplate> blocktemplate(BlockAssembler(active_chainstate, *node.evodb, llmq_ctx, *node.mn_subsidy, empty_mempool, chainparams).CreateNewBlock(coinbase_script));
        if (!blocktemplate) {
            throw JSONRPCError(RPC_INTERNAL_ERROR, "Couldn't create new block");
        }
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        pblocktemplate = BlockAssembler(active_chainstate, *node.evodb, llmq_ctx, *node.mn_subsidy, mempool, Params()).CreateNewBlock(scriptDummy);
        if (!pblocktemplate)
