...
void ProcessNetInfoPlatform(T1& ptx, const UniValue& input_p2p, const UniValue& input_http, const bool optional)
{
    ...
    auto process_field = [&](uint16_t& maybe_target, const UniValue& input, const NetInfoPurpose purpose) {
        ...
        if (is_empty || input.isArray() || (input.isStr() && !IsNumeric(input.getValStr()))) {
            ...
            if (!ptx.netInfo->CanStorePlatform()) {
                ...
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   strprintf("This ProTx's version allows port number only in %s",
                                             field_name));
            }
            if (is_empty && !ptx.netInfo->IsEmpty()) {
            ...
            if (input.isArray()) {
                ...
                parse_entry(ptx, input.get_str(), purpose, -1, true);
            }
        } else { // numeric str
