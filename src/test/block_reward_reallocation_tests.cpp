...
BOOST_FIXTURE_TEST_CASE(block_reward_reallocation, TestChainBRRBeforeActivationSetup)
{
...

    {
        ...
        const auto pblocktemplate = BlockAssembler(m_node.chainman->ActiveChainstate(), *m_node.evodb, *m_node.llmq_ctx, *m_node.mn_subsidy, *m_node.mempool, Params()).CreateNewBlock(coinbasePubKey);
    }
