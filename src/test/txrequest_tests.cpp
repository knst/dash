...
struct Runner
{
    /** The ObjRequestTracker being tested. */
    ObjRequestTracker txrequest;
...
/** A proxy for a Runner that helps build a sequence of consecutive test actions on a ObjRequestTracker.
 *
 ...
 * against a ObjRequestTracker in parallel. Every test has its own unique txhashes and NodeIds which are not
...
public:
    ...
    /** Schedule a ForgetObjHash call at the Scheduler's current time. */
    void ForgetObjHash(const uint256& txhash)
    {
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
            runner.txrequest.ForgetObjHash(txhash);
            runner.txrequest.SanityCheck();
        });
    }

    /** Schedule a ReceivedInv call at the Scheduler's current time. */
    void ReceivedInv(NodeId peer, const CInv& inv, bool pref, std::chrono::microseconds reqtime)
    {
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
            runner.txrequest.ReceivedInv(peer, inv, pref, reqtime);
            runner.txrequest.SanityCheck();
        });
    }
    ...
    /** Schedule a RequestedObj call at the Scheduler's current time. */
    void RequestedObj(NodeId peer, const uint256& txhash, std::chrono::microseconds exptime)
    {
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
            runner.txrequest.RequestedObj(peer, txhash, exptime);
            runner.txrequest.SanityCheck();
        });
    }
    ...
    /** Schedule calls to verify the ObjRequestTracker's state at the Scheduler's current time.
     *
     ...
     * where priority is the predicted internal ObjRequestTracker's priority, assuming all announcements
    ...
    /** Generate a random GenTxid; the txhash follows NewTxHash; the type is random between regular tx and CJ tx. */
    CInv NewGTxid(const std::vector<std::vector<NodeId>>& orders = {})
    {
        return {InsecureRandBool() ? MSG_TX : MSG_DSTX, NewTxHash(orders)};
    }
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    if (config >> 3) { // We'll request the transaction
        ...
        scenario.RequestedObj(peer, gtxid.hash, scenario.Now() + expiry);
        ...
        if ((config >> 3) == 1) { // The request will time out
            ...
            if ((config >> 3) == 3) { // A response will arrive for the transaction
                scenario.ReceivedResponse(peer, gtxid.hash);
    ...
    if (config & 4) { // The peer will go offline
        ...
        scenario.ForgetObjHash(gtxid.hash);
    }
...
void BuildPriorityTest(Scenario& scenario, int config)
{
    ...
    if (config & 8) {
        scenario.RequestedObj(priopeer, gtxid.hash, MAX_TIME);
    ...
    if (config & 16) {
        ...
        scenario.ReceivedResponse(priopeer, gtxid.hash);
    }
    if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
...
/** Add to scenario a test that verifies behavior related to both TX and DSTX with the same
...
void BuildWtxidTest(Scenario& scenario, int config)
{
    ...
    auto expiry = RandomTime8s();
    if (config & 2) {
        scenario.RequestedObj(peerT, txid.hash, scenario.Now() + expiry);
...
void BuildTimeBackwardsTest(Scenario& scenario)
{
    ...
    scenario.RequestedObj(peer1, gtxid.hash, expiry);
...
/** Add to scenario a test that involves RequestedObj() calls for txhashes not returned by GetRequestable. */
void BuildWeirdRequestsTest(Scenario& scenario)
{
    ...
    if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
    scenario.RequestedObj(peer1, gtxid2.hash, MAX_TIME);
    ...
    scenario.RequestedObj(peer1, gtxid1.hash, expiryA);
    ...
    scenario.RequestedObj(peer1, gtxid1.hash, expiryB);
    ...
    if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
    scenario.RequestedObj(peer1, gtxid1.hash, MAX_TIME);
    ...
    if (InsecureRandBool()) scenario.AdvanceTime(RandomTime8s());
    scenario.RequestedObj(peer2, gtxid2.hash, MAX_TIME);
...
BOOST_AUTO_TEST_CASE(ObjRequestTrackerTest)
{
    for (int i = 0; i < 5; ++i) {
