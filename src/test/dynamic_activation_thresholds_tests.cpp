...
struct TestChainDATSetup : public TestChainSetup
{
    ...
    void test(int activation_index, bool check_activation_at_min)
    {
...

        {
            ...
            const auto pblocktemplate = BlockAssembler(::ChainstateActive(), *m_node.evodb,  *m_node.llmq_ctx, *m_node.mn_subsidy, *m_node.mempool, Params()).CreateNewBlock(coinbasePubKey);
