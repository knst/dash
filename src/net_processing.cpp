...
void PeerManagerImpl::AddObjectAnnouncement(const CNode& node, const CInv& inv, std::chrono::microseconds current_time)
{
    ...
    //   - NONPREF_PEER_TX_DELAY for announcements of transactions (both MSG_TX and MSG_DSTX) from
    //     non-preferred connections. Dash-specific object types are never delayed (they carry
    //     time-sensitive consensus data), and neither is anything in masternode mode.
    ...
    auto delay{0us};
    if (inv.IsGenTxMsg() && !preferred && m_nodeman == nullptr) delay += NONPREF_PEER_TX_DELAY;
