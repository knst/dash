 ...
 * - The hash of the object (collectively called "objhash" or "txhash" in what follows)
 * - The name txhash is inherited from Bitcoin codebase but Dash implementation operates any type of inventory
 ...
 *              per object hash, so if the same objects is announced with different types, we have no means
 ...
 *     ForgetObjHash, which removes all announcements across all peers with the specified txhash.
...
public:
    //! Construct a ObjRequestTracker.
    explicit ObjRequestTracker(bool deterministic = false);
    ~ObjRequestTracker();
     ...
     * COMPLETED). Note that the CInv property is ignored for determining uniqueness, so if an announcement
     * is added for type W while one for type H from the same peer already exists, it will be ignored. This is
     * harmless as the hashes being equal, so it doesn't matter how it is
     * fetched. The new announcement is given the specified preferred and reqtime values, and takes its type 
     * from the specified CInv.
     */
    ...
    /** Deletes all announcements for a given txhash (any type).
     *
     ...
     */
    void ForgetObjHash(const uint256& objhash);
     ...
     *  - The selected announcements are converted to CInv using their type, and returned in
     ...
     */
    void RequestedObj(NodeId peer, const uint256& objhash, std::chrono::microseconds expiry);
     ...
     * not needed entirely anymore, ForgetObjHash should be called instead of, or in addition to, this call.
     */
    void ReceivedResponse(NodeId peer, const uint256& objhash);
