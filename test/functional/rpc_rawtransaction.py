...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def getrawtransaction_tests(self):
        ...
        for n in [0, 2]:
            ...
            if n == 0:
                ...
                assert_equal(self.nodes[n].getrawtransactionmulti({"0":[txId]}, 2)[txId]['hex'], tx['hex'])
            ...
            for value in ["True", "False"]:
                ...
                assert_raises_rpc_error(-3, "not of expected type number", self.nodes[n].getrawtransactionmulti, transactions={"0":[txId]}, verbose=value)
                assert_raises_rpc_error(-3, "not of expected type number", self.nodes[n].getrawtransactionmulti, transactions={"0":[txId]}, verbosity=value)
    ...
    def getrawtransaction_verbosity_tests(self):
        ...
        # Pre-DIP3 regtest blocks are small enough that the first 64 KiB block file only ends around height 365,
        # so prune above that while staying MIN_BLOCKS_TO_KEEP below the tip
        self.generate(self.nodes[2], 600)
        assert_greater_than(self.nodes[2].pruneblockchain(400), 0)
